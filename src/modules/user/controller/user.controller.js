...
refreshToken</a>`;
 //send email 
  if (!sendEmail({ to: email, subject: "confirm_email", html })) {
...
export const profile = asyncHandler(async (req, res, next) => {
  ...
  const profile = await userModel
    .findById({ _id: userId })
    .select("userName email gender DOB -_id");
  if (!profile) {
    return next(new Error("user not found", { cause: 404 }));
  }
...
export const updatePassword = asyncHandler(async(req,res,next)=>{
    ...
    if (userId != req.user._id) {
      return next(
        new Error("you are not the owner of account", { cause: 400 })
      );
    }
    //compare passwords
...
// Forget password ( without sending any email , make sure of your data security specially the OTP and the newPassword )
